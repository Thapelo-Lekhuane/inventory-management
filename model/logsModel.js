...
exports.saveLogDetail = function (handlerInfo, updateData, username, id, inventoryType, changeType, callback) {
    ...
    var sqlQuery = db.executeQuery(sql, data, function (err, upd) {
        if (err) {
            return callback(err);
        }
        callback();
...
exports.saveLogDetail = function (handlerInfo, updateData, username, id, inventoryType, changeType, callback) {
    ...
    var sqlQuery = db.executeQuery(sql, [], function (err, log) {
        if (err) {
            return callback(err);
        }
        if(!log.length) {
            var sql4 = 'INSERT INTO logs_pool SET ?';
            var sqlQuery4 = db.executeQuery(sql4, data, function (err4, upd4) {
                if (err4) {
            ...
            if((log[0].username === username) && (log[0].item_id === id) && (log[0].change_type === changeType)){
                ...
                var sqlQuery2 = db.executeQuery(sql2, [changeStr, log[0].id], function (err2, upd2) {
                    if (err2) {
                ...
                var sqlQuery3 = db.executeQuery(sql3, data, function (err3, upd3) {
                    if (err3) {
...
exports.getLogs = function (handlerInfo, userId, callback) {
    ...
    var sqlQuery = db.executeQuery(sql, values, function (err, logs) {
        if (err) {
            return callback(err);
        }
        if (!logs.length) {
