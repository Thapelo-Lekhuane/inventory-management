...
router.post('/create-item', function (req, res) {
    ...
    if (util.checkBlank([name, category, quantity, price])) {
...
router.get('/get-items', function (req, res) {
    ...
    async.waterfall(tasks, function (error, result) {
        ...
        console.log('Get items successful:', result);
...
router.post('/delete-item', function (req, res) {
    var handlerInfo = {
        apiModule: 'inventory',
        apiHandler: 'delete-item'
    };
    console.log('Delete item request body:', req.body);

    var itemId = req.body.item_id;

    if (util.checkBlank([itemId])) {
        return response.parameterMissing(handlerInfo, res);
    }

    var tasks = [
        __.deleteItem.bind(null, handlerInfo, itemId)
    ];
